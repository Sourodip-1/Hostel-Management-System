...
void initalizeAdminSystem() // initializing default admin
{
  ...
  // Try to open for reading to check if file exists and has data
  fp = fopen("data/admin.dat", "rb");
  int needsInit = 0;
  ...
  if (needsInit) {
    ...
    if (fp != NULL) {
      ...
      char plainPass[30] = "Test@123";
      hashPassword(plainPass,
                   defaultUser.password); // Hash the default password

      fwrite(&defaultUser, sizeof(struct admin), 1, fp);
      fclose(fp);
      printf("System Initialized with Default Admin (1000 / Test@123)\n");
    } else {
      printf("Error: Unable to create admin file.\n");
    }
  }
}
...
void addAdmin() {
  ...
  printf("Password: ");
  scanf("%29s", tempPass);
  hashPassword(tempPass, newAdmin.password);
...
int adminLogin(int adminId, char password[65]) {
  struct admin a;
  FILE *fp = fopen("data/admin.dat", "rb");
  ...
  while (fread(&a, sizeof(struct admin), 1, fp) == 1) {
    if (a.adminId == adminId) {
      if (strcmp(a.password, hashedInput) == 0) {
        fclose(fp);
